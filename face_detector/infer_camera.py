...
from face_detector.utils.utils import generate_bbox, py_nms, convert_to_square, pad, calibrate_box, processed_image
...
# 画出人脸框和关键点
def draw_face(img, boxes_c, expand_ratio=0.45):
    all_bbox = []
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret:
            # 预测图片获取人脸的box和关键点
            boxes_c, _ = infer_image(img)
            # 把关键画出来
            if boxes_c is not None:
                draw_face(img=img, boxes_c=boxes_c)
            else:
